...
const App: React.FC = () => {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden text-white bg-black selection:bg-cyan-500 selection:text-black font-sans">
      <Background />
      ...
      {/* Navigation Controls */}
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-4 bg-black/50 backdrop-blur-xl px-6 py-3 rounded-full border border-white/10">
        <button 
          onClick={prevSlide} 
          disabled={currentSlide === 0}
          className="p-2 hover:bg-white/10 rounded-full disabled:opacity-30 transition"
        >
          <ChevronLeft size={20} />
        </button>
        ...
        <button 
          ...
          className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider hover:text-cyan-400 transition disabled:opacity-50"
        >
          {isExporting ? (
